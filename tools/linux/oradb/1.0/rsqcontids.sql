...
mg.CONTAINER_ID as "container_id",
sum(decode(mg.status, 3, 1, 0)) as "error_groups",
sum(decode(mg.status, 1, 1, 0)) as "locked_groups"
...
SELECT 
to_char(sysdate,'YYYY-MM-DD HH24:MI:SS') as "time",
mg2.CONTAINER_ID as "container_id",
sum(decode(mg2.status, 3, 1, 0)) as "error_groups",
sum(decode(mg2.status, 1, 1, 0)) as "locked_groups"
